...
- For Example : Let's look up at the Instagram App
  1. The first screen when we open the app is the 'Main Activity'. Every app has it 'Main Activity'.
  2. Let's say now we move to any person's profile. The profile of the person will a different activity.
  3. Now let's say we move to some another person's profile page will it be an different activity too? Well! The answer is 'No'. All profile page for the person will have same layout. Hence, the profile for every person will be same.
  4. All the user's profile pages are one single activity is the content which gets changes.
  5. Now let's say in a person's profile we open their picture post then it's different activity. Hence, we can say all picture post pages will be same activity.
  6. The activities of an app are all related.
  7. Suppose in Instagram we move from Main Activity(Homepage/start screen of instagram) to some different Activity then the previous activity gets paused. 
     For Example : if we open instagram app we are redirected to homepage or start page then if we click on dm button on right top screen it redirects us to different activity which is dm page and the homepage activity will be paused for the instance after switching to dm page activity.

 - Some points about an activity: 
 1. An activity has different states, like create, pause, stop, destroy, etc.
 2. These states are attained by an activity because of some events. It is based on user's behaviour.
 ...
 - What is an Activity Lifecycle?
  ...
  -> As a user navigates the app and goes back and later comes back to the app. The activity instances in the app transitions through various various states in their lifecycles.
  -> So we write code in our app. The activity class has number of methods which are known as Callbacks. These methods are triggered when various events takes place in the activity.
 ...
 - Callbacks/methods in the activity lifecycle
  ...
  3. onResume()
    - This method is called when the activity is ready for interaction with user.
    - The activity remains in this state till the user is interacting with it.
    - It changes this state in case of interruptions. 
        For Example : When a phone call is coming or when the user navigates to some other activity of the same app or whe the device screen turns off. Hence when the interruption occurs the activity goes to pause state.
  ...
  4. onPause()
    ...
    - This state can change in two conditions:
      1. If the activity is resumed and is being used again. Once the activity is resumed then the onResume() method is called. Now the system keeps the activity instance that was last shown to the user in memory and recalls that instance when the onResume method is called.
         For Example : When you move from one app to another and then come back to first app you come back to the same screen(activity) where you had left-off. That is what we mean by returning to same instance.
      2. If the activity becomes completely invisible to the user. Now a if an activity wants to be completely invisible then the onStop() method will be called. 
  ...
  5. onStop()
    ...
    - This state may also be invoked by the system, if it needs more resources for other apps. For example, if the RAM of the device is low and it needs it for other functions it can directly stop the activity without sending it to pause state.
    - All resource must freed up in this state. For example, you get a call and you're on that call for a long time say for an hour. In this activity first goes into the pause state and eventually will get stopped(stop state).

  6. onDestroy()
    - This is the final call that activity receives.
    - This method may be invoked due to the user's actions such as when the user stops using the app and removes it from background also or it may be invoked by the system when it wants to free up space for some other important functions. You may have seen sometimes an app stops responding. Well that's because the activity faces a storage of resources and hence it is destroyed. 
